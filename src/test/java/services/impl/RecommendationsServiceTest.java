...
public class RecommendationsServiceTest extends BaseOfyTest {
	...
	public void createRecommendationTest() {
		...
		final OfyTripsService tripsService = new OfyTripsServiceImpl(userRepo,
			tripRepo,
			recommendationRepo);
		final OfyRecommendationsService recService = new OfyRecommendationsServiceImpl(userRepo,
			tripRepo,
			recommendationRepo);
		...
		Assert.assertEquals(0, recService.findAll()
			.size());
		recService.createRecommendation(user1.getId(),
			user2.getId(),
			trip.getId());
		Assert.assertEquals(1, recService.findAll()
			.size());
	}
	...
	public void findByIdTest() {
		...
		Assert.assertEquals(user1.getId(), recom.getOwner()
			.getId());
		Assert.assertEquals(user2.getId(), recom.getTarget()
			.getId());
	}
	...
	public void findByOwnerAndStatusTest() {
		...
		List<OfyRecommendation> lista = recService.findByOwnerAndStatus(user1.getId(),
			Recommendation.Status.PENDING);
		Assert.assertEquals(1, lista.size());
		List<OfyRecommendation> lista2 = recService.findByOwnerAndStatus(user1.getId(),
	...
	public void findByTargetAndStatusTest() {
		...
		List<OfyRecommendation> lista = recService.findByTargetAndStatus(user2.getId(),
			Recommendation.Status.PENDING);
		Assert.assertEquals(1, lista.size());
		List<OfyRecommendation> lista2 = recService.findByTargetAndStatus(user2.getId(),
			Recommendation.Status.ACCEPTED);
		Assert.assertEquals(0, lista2.size());
		List<OfyRecommendation> lista3 = recService.findByTargetAndStatus(user1.getId(),
			Recommendation.Status.PENDING);
		Assert.assertEquals(0, lista3.size());
	}

	@Test
	public void patchRecommendationTest() {
		...
		OfyRecommendation recom1 = recService.createRecommendation(user1.getId(),
			user2.getId(),
			trip1.getId());

		recommendationRepo.add(recom1);

		Assert.assertEquals(Recommendation.Status.PENDING,
			recom1.getStatus());

		recService.patchRecommendation(user2.getId(),
			recom1.getId(),
			Recommendation.Status.ACCEPTED);

		Assert.assertEquals(Recommendation.Status.ACCEPTED,
			recService.findById(recom1.getId())
				.getStatus());
