...
public class TripServiceTest extends BaseOfyTest {
	...
	public void createTripTest() {
		...
		Assert.assertEquals("Buenos Aires", trip.getTripDetails()
			.getFromCity()
			.getName());
		Assert.assertEquals("Roma", trip.getTripDetails()
			.getToCity()
			.getName());
	}
	...
	public void findAcceptedByTargetTest() {
		...
		final OfyRecommendationsService recommendationService = new OfyRecommendationsServiceImpl(userRepo,
			tripRepo,
			recommendationRepo);
		...
		usersService.createUser(new UserDetails(2L,
			"user2",
			"mail",
			"link"));
		...
		final Recommendation rec = recommendationService.createRecommendation(1L,
			2L,
			trip.getId());

		recommendationService.patchRecommendation(2L,
			rec.getId(),
			Status.ACCEPTED);

		List<? extends AcceptedTrip> acceptedTrips = tripsService.findAcceptedByTarget(2L);

		Assert.assertEquals(1, acceptedTrips.size());
		Assert.assertEquals(trip.getId(), acceptedTrips.get(0).getTripId());
	...
	public void findAllTest() {
		...
		Assert.assertFalse(userRepo.findAll().isEmpty());
		...
		Assert.assertTrue(!tripsService.findAll().isEmpty());
	...
	public void findByOwnerTest() {
		...
		Assert.assertEquals(user1, tripsService.findByOwner(1L)
			.get(0)
			.getOwner());
	}
	...
	public void findByIdTest() {
		...
		Assert.assertEquals(5545.12,
			detalles.getPriceDetail().getTotal(),
			0);
	}
	...
	public void deleteByIdTest() {
		...
		tripsService.findById(trip1.getId());
