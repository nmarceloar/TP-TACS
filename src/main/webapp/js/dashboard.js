...
var Trip = function(org, dst, start, end) {
	...
	for (var i = 0; i < this.outbound.segments.length; i++) {
	    this.outbound.segments[i].duration = dateDifference(
		    this.outbound.segments[i].departure_datetime,
		    this.outbound.segments[i].arrival_datetime).toString();
	    segments.push(this.outbound.segments[i]);
	}
	for (var i = 0; i < this.inbound.segments.length; i++) {
	    this.inbound.segments[i].duration = dateDifference(
		    this.inbound.segments[i].departure_datetime,
		    this.inbound.segments[i].arrival_datetime).toString();
...
var listIdAmigosARecomendar = []; // Se guardan pares id, nombre de los
// posibles a recibir recomendaciones
var amigosSelecRecomendar = []; // Se colocan los finalmente seleccionados para
// recomendar
...
$(function() {
    ...
    /*
     * TODO $("#drpDwnRecomendaciones").click(function(event){
     * event.preventDefault(); acá debería traer las últimas 5 notificaciones y
     * resalto las no leídas });
     * 
     * TODO $("#btnMarcarComoLeidas").click.... marcar las notificaciones como
     * leídas
     */
    ...
    $("#btnAceptarRecom").click(
		...
		$
			.ajax({
			    url : '/api/recommendations/one/' + recomActiva
				    + '?st=acp',
			    type : 'PUT',
			    dataType : 'text',
			    success : function(data) {
				console.log(data);
				bootbox.alert(
					'Se ha aceptado la recomendacion',
					function() {
					});
			    },
			    error : function(data) {
				console.log(data);
				bootbox.alert(
					'Fallo la aceptacion de recomendacion',
					function() {
					});
			    }
			});
			...
			$
				.each(
					...
					function(i, val) {
					    ...
					    console
						    .log('Recomendacion posteada ------');
					    console.log('usuario a postearle: '
						    + val);
					    console.log('usuario que postea: '
						    + id);
					    console
						    .log('id viaje recomendado: '
							    + idViajeARecomendar);
					    $
						    .ajax({
							type : 'POST',
							...
							success : function(data) {
							    ...
							    bootbox
								    .alert(
									    "Has recomendado el viaje satisfactoriamente",
									    function() {
									    });
							}
						    });
					    ...
					    // + val +
					    // '/notifications?template=Recibiste
					    // una recomendacion de un
					    // viaje&href=http://localhost:8080',
					    // // data: "template=Recibiste una
					    // recomendacion de un
					    // viaje&href=http://localhost:8080",
					    // // contentType:
					    // 'application/json',
    ...
    // TODO habría que hacer que los elementos se te vayan agregando en el input
    // (como el de facebook)
    $("#boxAmigos")
	    .autocomplete(
		    {
			...
			select : function(event, ui) {
				...
				if (listIdAmigosARecomendar[indice].name == ui.item.value) {
				    amigosSelecRecomendar
					    .push(listIdAmigosARecomendar[indice].id);
				}
			    }
			    $("#amigosList").append(
				    "<li>" + ui.item.value + "</li>");
    ...
    $("#btnBuscarVuelo").click(
		...
		currentTrip = new Trip(orgCity, dstCity,
			...
			$
				.ajax({
				    ...
				    data : JSON
						...
						"price" : currentTrip.price.total
							+ " "
							+ currentTrip.price.currency,
					...
					contViajes++;
					$("#listViajes").append(
						getViajeHTML(data.id));
					bootbox
						.confirm(
							"Felicitaciones por tu viaje! Queres publicarlo en tu muro de Facebook?",
							function(result) {
							    if (result) {
								FB
									.api(
...
function findLatestListOfFriends() {
...
function insertarRecomendacionesDeViajes(data) {
    ...
    return '<li><a href="#" role="linkViajeRecom" id="' + data.id + '" trip="'
	    + data.viajeAsoc + '">' + data.nombreyap
	    + ' quiere que viajes desde ' + data.origen + ' hasta '
	    + data.destino + '</a></li>';
}

function getTitleViaje(vuelo) {
    var canttramos = vuelo.segments.length;
    return ' el ' + vuelo.segments[0].departure_datetime + ' y llegando el '
	    + vuelo.segments[(canttramos - 1)].arrival_datetime
	    + ((canttramos > 1) ? ' en ' + canttramos + ' escalas' : '') + '.';
}

function getDescripcionViaje(i, viaje, sentido) {
    var canttramos = viaje.segments.length;
    var desc = '<li class="list-group-item" role="opcion-vuelo"><label><input type="radio" name="optradio-'
	    + sentido
	    + '" alternativa="'
	    + i
	    + '" class="vuelo-tramo-ajuste" />';
    for (var i = 0; i < canttramos; i++) {
	desc += '<p class="vuelo-tramos-detail">Vuelo '
		+ viaje.segments[i].flight_id + ' de '
		+ viaje.segments[i].airline + ' ' + 'saliendo de '
		+ viaje.segments[i].from + ' a las '
		+ viaje.segments[i].departure_datetime + ' ' + 'llegando a '
		+ viaje.segments[i].to + ' a las '
		+ viaje.segments[i].arrival_datetime + '.</p>';
    }
...
function templateVuelo(posData, vuelo) {
	...
	descripcionida += getDescripcionViaje(i, vuelo.outbound_choices[i],
		'ida');
    }

    var descripcionvuelta = '<li class="list-group-item active">Vuelta</li>';
    for (j = 0; j < vuelo.inbound_choices.length; j++) {
	descripcionvuelta += getDescripcionViaje(j, vuelo.inbound_choices[j],
		'vuelta');
    }

    return '<div class="list-group-item" role="vuelo" vuelo="' + posData + '">'
	    + '<a href="#" role="ver-detalle-vuelo" vuelo="' + posData
	    + '"><h4 class="list-group-item-heading">Vol\u00e1 por '
	    + vuelo.price_detail.total + vuelo.price_detail.currency
	    + '</h4></a>'
	    + '<div class="list-group" role="opciones-vuelo" vuelo="' + posData
	    + '" type="ida">' + descripcionida + '</div>'
	    + '<div class="list-group" role="opciones-vuelo" vuelo="' + posData
	    + '" type="vuelta">' + descripcionvuelta + '</div>' + '</div>';
}
// ** templates ************************************************
...
function getVuelos() {
    $
		...
		success : function(data) {
			...
			for (i = 0; i < datalen; i++) {
			    $("#listaVuelos").append(
				    templateVuelo(i, data.items[i]));
			}
			$("div[role=opciones-vuelo]").hide();
			$("a[role=ver-detalle-vuelo]").click(
				function(event) {
				    $(
					    "div[role=opciones-vuelo][vuelo!="
						    + $(this).attr("vuelo")
						    + "]").hide();
				    $(
					    "div[role=opciones-vuelo][vuelo!="
			...
			$("input[type=radio][alternativa]")
				.click(
					function(event) {
					    var i_vuelo = $(this).parents(
						    "div[role=opciones-vuelo]")
						    .attr("vuelo");
					    var i_alternativa = $(this).attr(
						    'alternativa');
					    var i_type = $(this).parents(
						    "div[role=opciones-vuelo]")
						    .attr("type");
						...
						var tam = currentTrip.outbound.segments.length - 1; // Maxima
						// posicion
						// de
						// segmento
						...
						for ( var x in airports) {
							...
							console
								.log('Encontre el aeropuerto: '
									+ airports[x].code
									+ ' en L: '
									+ airports[x].latitude
									+ ' Lon: '
									+ airports[x].longitude
									+ ' - '
									+ airports[x].descripcion);
							airportdata
								.push({
								    "latitude" : airports[x].latitude,
								    "longitude" : airports[x].longitude,
								    "descripcion" : airports[x].descripcion
								});
						    }
						}
						for ( var x in airports) {
...
function initClickDetalle(id) {
    ...
    // reviso si la lista de recomendaciones está abierta y la cierro si hace
    // falta
    if (typeof $("#modListaRecomendaciones").data("bs.modal") != 'undefined'
	    && $("#modListaRecomendaciones").data("bs.modal").isShown) {
	...
	success : function(data) {
		...
		if (fechaSalida.toString("dd/MM/yyyy") == fechaLlegada
			.toString("dd/MM/yyyy")) {
		    itinerario = itinerario + "Sal\u00eds el "
			    + fechaSalida.toString("dd/MM/yyyy") + " desde "
			    + value.from + " a las "
			    + fechaSalida.toString("HH:mm") + " hs "
			    + "y lleg\u00e1s a " + value.to + " a las "
			    + fechaLlegada.toString("HH:mm")
			    + " del mismo d\u00eda";
		} else {
		    itinerario = itinerario + "Sal\u00eds el "
			    + fechaSalida.toString("dd/MM/yyyy") + " desde "
			    ...
			    + fechaSalida.toString("HH:mm") + " hs "
			    + "y lleg\u00e1s el "
			    + fechaSalida.toString("dd/MM/yyyy") + " a "
			    + value.to + " a las "
			    + fechaLlegada.toString("HH:mm");
		}
...
function initClickDetalleRecom(datos) {
    ...
    console.log('click detalle: idViaje: ' + idViaje + ' - recom: '
	    + recomActiva);
    ...
    var hasta;
    $.ajax({
	url : '/api/me/created-trips' + '/' + idViaje,
	dataType : 'json',
	success : function(data) {
	    console.log(data);
	    precio = data.price;
	    ...
	    $("div[id=modDetalleViajeRecom] div[class=col-md-6]").html(
		    itinerario);
	    // TODO llenar el mapa de detalle del viaje
...
function initClickCompartir(idViajeACompartir) {
    ...
    $
		...
		success : function(data) {
		    ...
		    bootbox
			    .confirm(
				    "Queres publicar este viaje en tu muro de Facebook?",
				    function(result) {
					if (result) {
					    FB
						    .api(
...
function initialize() {
    ...
    mapaNuevoViaje = new google.maps.Map(document
	    .getElementById("googleMapViaje"), mapProp);
    mapaVuelo = new google.maps.Map(document.getElementById("googleMapVuelo"),
	    mapProp);
    mapaReview = new google.maps.Map(document
	    .getElementById("googleMapViajeReview"), mapProp);
    mapaReviewRecom = new google.maps.Map(document
	    .getElementById("googleMapViajeReviewRecom"), mapProp);
...
function drawFlightRoute(airportdata) {
	...
	var marker = setMapMarker(currentMap, airportdata[i].latitude,
		airportdata[i].longitude, airportdata[i].description);
...
function updateStatusCallback(response) {
	...
	$.ajax({
		...
		if (data.length != 0) {
			...
			$("div[id=" + value.id + "] a[role=linkViaje]").click(
				value.id, initClickDetalle);
			$(
				"div[id=" + value.id
					+ "] button[id=btnRecomendarViaje]")
				.click(value.id, initClickRecomendar);
			$("div[id=" + value.id + "] a[id=eliminarViaje]")
				.click(value.id, initClickEliminar);
			$("div[id=" + value.id + "] a[id=compartirViaje]")
				.click(value.id, initClickCompartir);
		    });
		}
	    }
	});
	 ...
	 */
	$.ajax({
	    url : '/api/me/friends-trips',
	    dataType : 'json',
	    success : function(data) {
		if (data.length !== 0) {
		    $.each(data, function(index, value) {
			$("#listViajesAmigos").append(
				getViajesDeAmigosHTML(value));
			$("div[id=" + value.idTrip + "] a[role=linkViaje]")
				.click(value.idTrip, initClickDetalle);
			$(
				"div[id=" + value.idTrip
					+ "] button[id=btnRecomendarViaje]")
				.click(value.idTrip, initClickRecomendar);
			$("div[id=" + value.idTrip + "] a[id=eliminarViaje]")
				.click(value.idTrip, initClickEliminar);
			$("div[id=" + value.idTrip + "] a[id=compartirViaje]")
				.click(value.idTrip, initClickCompartir);
		    });
		}
	    }
	});
			...
			if (data.length != 0) {
			    $
				    .each(
						...
						$("#listRecomendaciones")
							.append(
								insertarRecomendacionesDeViajes(value));
						// <a href="#"
						// role="linkViajeRecom"
						// id="itemRecom" trip="4">
						$(
							"a[role=linkViajeRecom][id="
								+ value.id
								+ "][trip="
								+ value.viajeAsoc
								+ "]").click({
						    "viaje" : value.viajeAsoc,
						    "idRec" : value.id
						}, initClickDetalleRecom);
					    });
			    $("#listRecomendaciones").append(
				    "<li class=\"divider\"></li>");
			    $("#listRecomendaciones")
				    .append(
					    "<li><a href=\"#\" id=\"verTodasRecomendaciones\">Ver todas las recomendaciones</a></li>");
			}
		    }
...
function getViajeParaFB(from, to, salida, vuelta) {
    return 'TACS POR EL MUNDO: Viajo desde ' + from + ' a ' + to
	    + ' saliendo el dia ' + salida + ' y volviendo el dia ' + vuelta;
}
// PUBLICAR EN MURO CUANDO SE CREA UN VIAJE
function publicar() {
    // pruebo las cosas del mapa
    console.log("https://maps.googleapis.com/maps/api/staticmap?center="
	    + mapaVuelo.getCenter().toUrlValue() + "&zoom="
	    + mapaVuelo.getZoom() + "&maptype=" + mapaVuelo.getMapTypeId()
	    + "&size=600x400" + "&markers=color:green%7C"
	    + markerOrigen.getPosition().toString().trim() + "%7C"
	    + markerDestino.getPosition().toString().trim()
	    + "&path=color:red%7C"
	    + markerOrigen.getPosition().toString().trim() + "%7C"
	    + markerDestino.getPosition().toString().trim());
    ...
    FB
	    .api(
			...
			message : getViajeParaFB(
				...
				currentTrip.outbound.segments[0].departure_datetime,
				currentTrip.inbound.segments[(currentTrip.inbound.segments.length - 1)].arrival_datetime),
			picture : "https://maps.googleapis.com/maps/api/staticmap?center="
				+ mapaVuelo.getCenter().toUrlValue()
				+ "&zoom="
				+ mapaVuelo.getZoom()
				+ "&maptype="
				+ mapaVuelo.getMapTypeId()
				+ "&size=600x400"
				+ "&markers=color:green%7C"
				+ markerOrigen.getPosition().toString().trim()
				+ "%7C"
				+ markerDestino.getPosition().toString().trim()
				+ "&path=color:red%7C"
				+ markerOrigen.getPosition().toString().trim()
...
function compartir(viaje) {
    ...
    // posteo en muro de facebook
    FB.api('/' + id + '/feed', 'post', {
	message : getViajeParaFB(viaje.fromCity, viaje.toCity,
		viaje.tripDepartureDate.toString("dd/MM/yyyy HH:mm"),
		viaje.tripArrivalDate.toString("dd/MM/yyyy HH:mm")),
	// picture: "https://maps.googleapis.com/maps/api/staticmap?center=" +
	// mapaVuelo.getCenter().toUrlValue() +
	// "&zoom=" + mapaVuelo.getZoom() +
	// "&maptype=" + mapaVuelo.getMapTypeId() +
...
// Salís el 23/04/2015 desde Ezeiza, Buenos Aires a las 14:35 hs y llegás al
// Aeropuerto Internacional de la ciudad de Panamá a las 23:35 del mismo día

function llegadaDatosViaje(datos) {
    $("#detViajeRecom").append(
	    'Salio el ' + datos.tripDepartureDate + ' desde ' + datos.fromCity
		    + ' hasta ' + datos.toCity + ' llegando el dia '
...
// $.getJSON("/api/me/created-trips/" + viajeId,
// llegadaDatosViaje);
// $("#modDetalleViajeRecom").modal('show');
// }
//
