...
$(function() {
	...
	$("#btnRecomendar").click(function(event) {
		...
		$.each(amigosSelecRecomendar, function(i, val) {
			...
			// data: "template=Recibiste una recomendacion de un
...
function initClickDetalle(event) {
	...
	console.log('click detalle: tipo ' + tipoDet + ' - idViaje: ' + idDet
			+ ' - recom: ' + recomActiva);
...
function drawFlightRoute(airportdata) {
	...
	for (var i = 0; i < airportdata.length; i++) {
		var marker = setMapMarker(currentMap, airportdata[i].geolocation,
				airportdata[i].description);
				...
				.ajax({
					...
					success : function(data) {
						...
						$.each(data, function(index, value) {
							...
							// <a href="#" role="linkViajeRecom" id="itemRecom"
							// trip="4">
							$(
									"a[trip=" + value.viajeAsoc
											+ "][role=linkViajeRecom]").click({
