...
public class PersistenceService implements PassengerAPI, TripsAPI,
    ...
    private void assignFacebookFriendsToPassenger(Passenger pasajero) {
        ...
        WebTarget target = client.target("https://graph.facebook.com/v2.3/"
                + pasajero.getIdUser()
        ...
        for (UserRegisteredFB fbUs : busqueda.getUsuarios()) {
            if ((this.psjDao.getPasajeroById(fbUs.getId()) != null)
                    && (this.sonAmigos(pasajero.getIdUser(), fbUs.getId()) == false)) {
    ...
    public void assignStateRecommendation(int idRec, String state) {
        ...
        if (state.equals("acp")) {
            ...
            Trip newTrip = new Trip(rec.getIdUsuarioRecom(),
                    viajeRecom.getFromCity(), viajeRecom.getToCity(),
                    viajeRecom.getPrice(), viajeRecom.getItinerary());
    ...
    private Passenger createPassengerToPost(String id, String shortToken) {
        ...
        for (Passenger p : this.psjDao.getTodosLosPasajeros()) {
            if (p.getIdUser()
                ...
                ClientConfig config = new ClientConfig().register(new JacksonFeature());
                Client client = ClientBuilder.newClient(config);
                WebTarget target = client.target("https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=1586547271608233&client_secret=359a6eae58ad036b4df0c599d0cdd11a&fb_exchange_token="
                        + shortToken);
                Invocation.Builder invocationBuilder = target.request();
                Response response = invocationBuilder.get();
        ...
        if (buscado == null) {
            ...
            Passenger pasajero = new Passenger(id, nombre.getFirst_name(),
    ...
    public void instanceAndSaveRecommendation(RecommendationBeanFB recBean,
        ...
        Recommendation rec = new Recommendation(idUser, idUsuarioQueRecomienda,
                psj.getName() + ' ' + psj.getSurname(), viaje.getFromCity(),
                viaje.getToCity(), idViajeRecom);
