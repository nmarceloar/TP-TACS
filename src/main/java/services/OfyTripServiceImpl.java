...
public class OfyTripServiceImpl implements OfyTripService {
    ...
    public OfyTrip createTrip(final long userId,
        ...
        return ofy().transact(new Work<OfyTrip>() {
            
            @Override
            public OfyTrip run() {
            
                OfyTrip trip =
                    new OfyTrip(userRepo.findById(userId), tripDetails);
    ...
    public List<AcceptedOfyTrip> findAcceptedByTarget(final long userId) {
        ...
        List<AcceptedOfyTrip> trips =
            Lists.transform(this.recommendationRepo.findByTargetAndStatus(
                this.userRepo.findById(userId), Status.ACCEPTED),
                new Function<OfyRecommendation, AcceptedOfyTrip>() {
                    
                    @Override
                    public AcceptedOfyTrip
                        apply(OfyRecommendation acceptedRecommendation) {
